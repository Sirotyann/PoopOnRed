...
func one_sec_passed():
	passed_time += 1
	if passed_time == 1:
		add_car_to_path(white_suv.instantiate(), $Container/Path3D)
	if passed_time == 10: 
		add_car_to_path(delivery_truck.instantiate(), $Container/Path3D)
...
func add_delivery_truck():
	...
	$Container/Path3D.add_child(delivery_truck_follow)
...
func add_suv():
	...
	$Container/Path3D.add_child(white_suv_follow)
